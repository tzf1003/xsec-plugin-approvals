...
export function activate(host) {
  ...
  function build() {
    root.replaceChildren(); addStyles(root); const page = element("section", "xsec-approvals"); const summary = element("div", "approval-card approval-summary"); const card = element("section", "approval-card"); const head = element("header", "approval-card-head"); const refresh = element("button", "approval-button", "刷新"); refresh.type = "button"; refresh.onclick = () => void refresh(); head.append(element("h1", "approval-card-title", "本会话审批记录"), refresh);
